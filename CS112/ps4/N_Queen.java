...
/**
 * NQueens - blueprint class for an object that solves the n-queens puzzle using
 * recursive backtracking. Includes a main() method for using the solver.
 */
public class NQueens {
    private boolean[][] board; // 2-D array that keeps track of which squares on the board are occupied
    ...
    /**
     * Constructor - initializes the state of a chess board with the specified boardSize
     */
    public NQueens(int n) {
        ...
        /*
         * set the entries in the the extra bookkeeping arrays to true, since initially everything is empty
         */
    ...
    /*
     * placeQueen - place a queen at the specified row and column; a private helper
     * method, so we don't need to perform error-checking!
     */
    ...
    /*
     * removeQueen - remove the queen at the specified row and column; a private
     * helper method, so we don't need to perform error-checking!
     */
    ...
    /*
     * isSafe - returns true if it is safe to place a queen at the specified square,
     * and false otherwise; a private helper method, so we don't need to perform
     * error-checking!
     */
    private boolean isSafe(int row, int col) {
        return (this.colEmpty[col] && this.upDiagEmpty[row + col]
                && this.downDiagEmpty[(this.board.length - 1) + row - col]);
    }

    /*
     * findSolution(int row) - recursive function to search for solutions. Tries to
     * find a safe column in the specified row. If it finds one, it makes a
     * recursive call to handle the next row. If it can't find one, it backtracks by
     * returning from the recursive call. a private helper method, so we don't need
    ...
    /*
     * findSolution() - public "wrapper" method that takes no parameters. Makes the
     * initial call to the private recursive-backtracking method, and returns
     * whatever it returns.
     * 
     * This is an example of method *overloading* -- since we have two methods with
