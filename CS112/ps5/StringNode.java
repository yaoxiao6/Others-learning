 ...
 * many of the methods, and it also allows the methods to handle empty strings,
 * which are represented using a value of null.
 */
public class StringNode {
     ...
     * given linked-list string. If the string is too short or if the user passes in
     * a negative i, the method returns null.
     */
     ...
     * linked-list string, where the first character has index 0. If the index i is
     * < 0 or i > length - 1, the method will end up throwing an
    ...
    /**
     * convert - converts a standard Java String object to a linked-list string and
     * returns a reference to the linked-list string
     */
    ...
    public static StringNode copy(StringNode str) {
        StringNode travStr = str;
        StringNode copyMe = new StringNode(travStr.ch, null);
        if (travStr.next != null) {
            travStr = travStr.next;
        }
        StringNode headCopy = copyMe;
        StringNode travCopy = headCopy;
        for (; travStr != null; travStr = travStr.next) {
            copyMe = new StringNode(travStr.ch, null);
            travCopy.next = copyMe;
            travCopy = travCopy.next;
        }
        return headCopy;

        // StringNode trav = str;
        // StringNode copyMe = null;
        // StringNode[] copyMeList = new StringNode[length(str)];
        // StringNode head = null;
        // int i = 0;
        // while (trav != null) {
        // copyMe = new StringNode(trav.ch, null);
        // copyMeList[i] = copyMe;
        // i++;
        // trav = trav.next;
        // }
        // head = copyMeList[0];
        // for (int j = 0; j < i + 1; j++) {
        // copyMeList[j].next = copyMeList[j + 1];
        // }
        // // StringNode copyMe = new StringNode(trav.ch, trav.next);
        // return head;
    ...
    /**
     * deleteChar - deletes character i in the given linked-list string and returns
     * a reference to the resulting linked-list string
     */
    ...
    /**
     * indexOf - returns the position of the first occurrence of character ch in the
     * given linked-list string. If there is none, returns -1.
     */
    ...
    /**
     * insertAfter - inserts the specified new character (newChar) after the first
     * occurrence of the specified character (afterChar) in the linked-list string
     * to which str refers. If afterChar is not in the string, the method adds the
     * character to the end of the string. Returns a reference to the first node in
     * the modified linked list, because the first node will change if the original
     * string was empty.
     */
    public static StringNode insertAfter(StringNode str, char newChar, char afterChar) {
        ...
        if (str.ch == afterChar) {
            StringNode newCharNode = new StringNode(newChar, str.next);
            str.next = newCharNode;
        } else {
     ...
     * position i of the specified linked-list string. Returns a reference to the
     * resulting linked-list string.
     */
    ...
    /**
     * insertSorted - inserts character ch in the correct position in a sorted list
     * of characters (i.e., a sorted linked-list string) and returns a reference to
     * the resulting list.
     */
    ...
    /**
     * isPrefix - determines if the linked-list string specified by prefix is a
     * prefix of the linked-list string specified by str -- i.e., if str starts with
     * the characters in prefix. It returns true if str starts with prefix and false
     * otherwise.
     */
    public static boolean isPrefix(StringNode prefix, StringNode str) {
        StringNode travPrefix = prefix;
        StringNode travStr = str;
        while (travPrefix != null) {
            if (travStr == null) {
                return false;
            }
            if (travPrefix.ch != travStr.ch) {
                return false;
            }
            travPrefix = travPrefix.next;
            travStr = travStr.next;
        }
        return true;
    }
    ...
    /**
     * length - recursively determines the number of characters in the linked-list
     * string to which str refers
     */
    ...
    /**
     * numOccur - find the number of occurrences of the character ch in the linked
     * list to which str refers
     */
    ...
    /**
     * removeAllSpaces - removes all spaces from the linked-list string to which str
     * refers. Modifies the linked list itself, rather than creating a new list.
     * Returns a reference to the first node in the modified linked list, because
     * the first node will change if the old first node contained a space.
     */
    public static StringNode removeAllSpaces(StringNode str) {
        StringNode travStr = str;
        StringNode copyMe = new StringNode(' ', null);
        StringNode headCopy = copyMe;
        StringNode travCopy = headCopy;
        for (; travStr != null; travStr = travStr.next) {
            if (travStr.ch != ' ') {
    ...
    /*
     * toString - creates and returns the Java string that the current StringNode
     * represents. Note that this method -- unlike the others -- is a non-static
     * method. Thus, it will not work for empty strings, since they are represented
     * by a value of null, and we can't use null to invoke this method.
     */
     ...
     * string to upper case. Modifies the list itself, rather than creating a new
     * list.
     */
    ...
    public static void main(String[] args) {
        ...
        System.out.print("Here it is with spaces removed: ");
