...
public class LLList implements List {
    ...
    /* modify the internals of the list so that the elements are reversed. */
    public void reverse() {
        Node newHead = new Node(null, null);
        Node temp = head;
        Node rest;
        for (Node trav = head.next; trav != null; trav = rest) {
            rest = trav.next;
            trav.next = newHead.next;
            newHead.next = trav;
        }
